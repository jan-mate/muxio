...
@Inject
constructor(
    @ApplicationContext private val context: Context, // Make context a private property
    ...
    fun ensureEssentialPlaylistsExist() {
        viewModelScope.launch(Dispatchers.IO) {
            val library = musicRepository.library ?: return@launch
            val playlistsToCheck = listOf("Favorites", "Trash")

            playlistsToCheck.forEach { playlistName ->
                val playlistExists = library.playlists.any { it.name.resolve(context) == playlistName }
                if (!playlistExists) {
                    musicRepository.createPlaylist(playlistName, emptyList())
                    L.d("Created '$playlistName' playlist")
                }
            }
        }
    }
    ...
    fun isSongInPlaylist(song: Song, playlistName: String): Boolean {
        val playlist = musicRepository.library?.playlists?.find { it.name.resolve(context) == playlistName }
        return playlist?.songs?.any { it.uid == song.uid } ?: false
    }

    fun addSongToPlaylist(song: Song, playlistName: String) {
        ensureEssentialPlaylistsExist()
        val playlist = musicRepository.library?.playlists?.find { it.name.resolve(context) == playlistName }
        if (playlist != null) {
            viewModelScope.launch {
                musicRepository.addToPlaylist(listOf(song), playlist)
            }
        }
    }
    ...
    fun removeSongFromPlaylist(song: Song, playlistName: String) {
        ...
        val playlist = musicRepository.library?.playlists?.find { it.name.resolve(context) == playlistName }
        if (playlist != null) {
            val updated = playlist.songs.filterNot { it.uid == song.uid }
            viewModelScope.launch {
                musicRepository.rewritePlaylist(playlist, updated)
            }
        }
    }
