...
class PlaylistDetailListAdapter(private val listener: Listener) :
    ...
    interface Listener : DetailListAdapter.Listener<Song>, EditableListListener {
        ...
        fun onStartEdit()
        override fun onPlayNow(item: Song)
    }
...
private constructor(private val binding: ItemEditableSongBinding) :
    ...
    fun bind(song: Song, listener: PlaylistDetailListAdapter.Listener) {
        ...
        binding.playSong.setOnClickListener { listener.onPlayNow(song)
    }
    }
    ...
    override fun updateEditing(editing: Boolean) {
        ...
        val margin = if (editing) {
            (30 * binding.root.context.resources.displayMetrics.density).toInt()
        } else {
            0
        }

        (binding.playSong.layoutParams as ViewGroup.MarginLayoutParams).apply {
            marginEnd = margin
        }

        binding.playSong.requestLayout()
    }
