...
class ArtistDetailFragment : DetailFragment<Artist, Music>() {
    ...
    override fun onRealClick(item: Music) {
        when (item) {
            ...
            is Song -> playbackModel.playNext(item)
    ...
    override fun onPlayNow(item: Music) {
        if (item is Song) {
            playbackModel.play(item, detailModel.playInArtistWith)
        } else {
            error("onPlayNow should only be called with a Song.")
        }
    }
