...
class QueueFragment : ViewBindingFragment<FragmentQueueBinding>(), EditClickListListener<Song> {
    ...
    override fun onClick(item: Song, viewHolder: RecyclerView.ViewHolder) {
        ...
        queueModel.playNext(viewHolder.bindingAdapterPosition)
    }

    override fun onGotoClicked(item: Song, viewHolder: RecyclerView.ViewHolder) {
        // This calls the ViewModel’s goto(adapterIndex) using the
        // adapter position of the clicked item
        queueModel.goto(viewHolder.bindingAdapterPosition)
    }
